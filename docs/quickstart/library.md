...
After installation the top-level `memdiver` package re-exports a small, supported public
API for opening dumps, converting them to the `.msl` container format, parsing containers,
and running analysis — without reaching into internal modules.
...
NiceGUI/Marimo UIs live behind the `[api]`, `[mcp]`, `[nicegui]`, `[marimo]` extras.
...
:::{tip}
The heavier orchestration (consensus/variance construction, candidate reduction, oracle
wiring) that the CLI's `auto-floor` subcommand assembles lives in `memdiver.engine.*`. For
an end-to-end run driven by config, prefer the CLI (`memdiver auto-floor …`) or the HTTP
API; use `run_auto_floor` directly when you already hold a variance array and an oracle.
:::
...
stack. The FastAPI web UI (`[api]`), MCP server (`[mcp]`), and the NiceGUI (`[nicegui]`) /
Marimo (`[marimo]`) UIs are opt-in extras (`[all]` installs every interface). The
