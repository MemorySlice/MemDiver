  ...
  const handleAutoDetect = async () => {
    ...
    try {
      ...
      if (res.match) {
        ...
        setDetectResult(t("list.matched", { name: res.match.name, confidence: (res.match.confidence * 100).toFixed(0) }));
      } else {
        setDetectResult(t("list.noMatch"));
      }
    } catch {
      setDetectResult(t("list.detectionFailed"));
    } finally {
      setDetecting(false);
    }
  };
    ...
    <div className="p-3 text-xs space-y-3" data-tour-id="structures-panel">
      ...
      <div className="flex gap-1">
        <button
          ...
          title={!canApply ? t("list.autoDetectNoCursor") : t("list.autoDetectTitle")}
        >
          {detecting ? t("list.autoDetectBusy") : t("list.autoDetectIdle")}
        </button>
      </div>
        ...
        grouped.map(([protocol, items]) => (
          <div key={protocol}>
            <div className="font-medium md-text-secondary uppercase tracking-wider text-[10px] mb-1">
              {protocol === "general" ? t("list.generalGroup") : protocol}
            </div>
              ...
              {items.map((s) => (
                <div key={s.name} className="flex items-start gap-2 py-1 px-1 rounded hover:bg-[var(--md-bg-hover)]">
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="ml-2 md-text-muted">{t("list.fieldSummary", { size: s.total_size, count: s.field_count })}</span>
                    {s.description && (
                  ...
                  <button
                    ...
                    title={canApply ? t("list.applyAtOffset", { offset: (cursorOffset ?? 0).toString(16) }) : t("list.applyNoCursor")}
                  >
                    {applyingName === s.name ? "..." : "\u25B6"}
                  </button>
                  <button onClick={() => downloadJson(s.name)} className="px-1 hover:md-text-accent" title={t("list.exportTitle")}>
                    &#x2913;
                  </button>
                  <button onClick={() => handleDelete(s.name)} className="px-1 hover:text-[var(--md-accent-red)]" title={t("common:delete")}>
                    x
                  </button>
                </div>
              ))}
