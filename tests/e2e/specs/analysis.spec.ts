...
test.describe("Analysis bottom tab mounts", { tag: "@requires-dataset" }, () => {
  ...
  test("mounts without errors", async ({ page }) => {
    ...
    await page.locator(tab("analysis")).first().click();
    await expect(page.locator(tab("analysis")).first()).toBeVisible();
