...
test.describe("Experiment bottom tab mounts", { tag: "@requires-dataset" }, () => {
  ...
  test("mounts without errors", async ({ page }) => {
    ...
    await page.locator(tab("experiment")).first().click();
    await expect(page.locator(tab("experiment")).first()).toBeVisible();
