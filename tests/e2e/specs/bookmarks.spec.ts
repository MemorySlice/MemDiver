...
test.describe("Bookmarks side tab mounts", { tag: "@requires-dataset" }, () => {
  ...
  test("mounts without errors", async ({ page }) => {
    ...
    await page.locator(tab("bookmarks")).first().click();
    await expect(page.locator(tab("bookmarks")).first()).toBeVisible();
