...
from memdiver.engine.project_db import check_deps, default_db_path, install_hint, HAS_DUCKDB
...
def test_default_db_path_creates_parent(tmp_path, monkeypatch):
    ...
    from memdiver.engine import project_db
    monkeypatch.delenv("XDG_DATA_HOME", raising=False)
...
def test_should_show_wizard_not_when_duckdb_installed():
    ...
    from memdiver.ui.components.setup_wizard import should_show_wizard
    if HAS_DUCKDB:
...
def test_should_show_wizard_always_false(tmp_path, monkeypatch):
    ...
    import memdiver.ui.components.setup_wizard as wiz
    monkeypatch.setattr(wiz, "_config_path", lambda: tmp_path / "config.json")
    monkeypatch.setattr("memdiver.engine.project_db.check_deps", lambda: {"ready": True})
...
def test_create_wizard_buttons():
    ...
    from memdiver.ui.components.setup_wizard import create_wizard_buttons
